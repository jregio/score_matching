...
class Distribution:
    def __init__(self, name, coeffs, means, variances):
        ...
        if len(coeffs) == 0:
            raise ValueError('Enter at least one set of parameters')
        ...
        for i in range(self.num):
            dist = torch.distributions.multivariate_normal.MultivariateNormal(means[i], torch.eye(self.dim) * variances[i])
    ...
    def generate_samples(self, num_samples):
        samples = [self.sample() for i in range(num_samples)]
